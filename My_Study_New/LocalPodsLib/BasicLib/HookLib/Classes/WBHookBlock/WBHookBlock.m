...
static void wbhook_block_setTmpBlock(WBFishBlock block, WBFishBlock tmpBlock) {
    objc_setAssociatedObject((__bridge id)block, @"wbhook_block_TmpBlock", (__bridge_transfer id)tmpBlock, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
}
...
static void wb_block_forwardInvocation(id self, SEL _cmd, NSInvocation *invo) {
    ...
    if (hookArgNum > 1) {
        ...
        // fix mem leaks    2021.2.2 by Howard
        [hookBlockInv retainArguments];   // retain 所有参数， 防止参数被释放
