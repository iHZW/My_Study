...
#define kEHIndexViewSpace (self.configuration.indexItemHeight + self.configuration.indexItemsSpace)
#define kEHIndexViewMargin ((self.bounds.size.height - kEHIndexViewSpace * self.dataSource.count) / 2)
#define kEHIndexViewInsetTop (self.translucentForTableViewInNavigationBar ? UIApplication.sharedApplication.statusBarFrame.size.height + 44 : 0)

static NSTimeInterval kAnimationDuration = 0.25;

// 根据section值获取TextLayer的中心点y值
static inline CGFloat EHGetTextLayerCenterY(NSUInteger position, CGFloat margin, CGFloat space)
{
    return margin + (position + 1.0 / 2) * space;
}

// 根据y值获取TextLayer的section值
static inline NSInteger EHPositionOfTextLayerInY(CGFloat y, CGFloat margin, CGFloat space)
{
    CGFloat position = (y - margin) / space - 1.0 / 2;
    if (position <= 0) return 0;
    NSUInteger bigger = (NSUInteger)ceil(position);
    NSUInteger smaller = bigger - 1;
    CGFloat biggerCenterY = EHGetTextLayerCenterY(bigger, margin, space);
    CGFloat smallerCenterY = EHGetTextLayerCenterY(smaller, margin, space);
    return biggerCenterY + smallerCenterY > 2 * y ? smaller : bigger;
}


#pragma mark - EHTextLayer
@interface EHTextLayer : CATextLayer

@property (nonatomic, strong) UIFont *itemFont;

@end

@implementation EHTextLayer
...
#pragma mark - EHIndexView

@interface EHIndexView ()

@property (nonatomic, strong, nullable) CAShapeLayer *searchLayer;
@property (nonatomic, strong) NSMutableArray<EHTextLayer *> *subTextLayers;
...
- (void)layoutSubviews {
    ...
    CGFloat space = kEHIndexViewSpace;
    CGFloat margin = kEHIndexViewMargin;
    
    [CATransaction begin];
    [CATransaction setDisableActions:YES];
    if (self.searchLayer && !self.searchLayer.hidden) {
        self.searchLayer.frame = CGRectMake(self.bounds.size.width - self.configuration.indexItemRightMargin - self.configuration.indexItemHeight, EHGetTextLayerCenterY(0, margin, space) - self.configuration.indexItemHeight / 2, self.configuration.indexItemHeight, self.configuration.indexItemHeight);
    ...
    for (int i = 0; i < self.subTextLayers.count; i++) {
        EHTextLayer *textLayer = self.subTextLayers[i];
        NSUInteger section = i + deta;
        textLayer.frame = CGRectMake(self.bounds.size.width - self.configuration.indexItemRightMargin - self.configuration.indexItemHeight, EHGetTextLayerCenterY(section, margin, space) - self.configuration.indexItemHeight / 2, self.configuration.indexItemHeight, self.configuration.indexItemHeight);
    }
...
- (void)configSubLayersAndSubviews
{
    ...
    if (countDifference > 0) {
        for (int i = 0; i < countDifference; i++) {
            EHTextLayer *textLayer = [EHTextLayer layer];
            [self.layer addSublayer:textLayer];
            [self.subTextLayers addObject:textLayer];
        }
    } else {
        for (int i = 0; i < -countDifference; i++) {
            EHTextLayer *textLayer = self.subTextLayers.lastObject;
    ...
    if (self.subTextLayers.count == 0) {
        ...
        self.currentSection = self.searchLayer ? EHIndexViewSearchSection : 0;
    } else {
...
{
    NSInteger currentSection = EHIndexViewInvalidSection;
    if (self.delegate && [self.delegate respondsToSelector:@selector(sectionOfIndexView:tableViewDidScroll:)]) {
        ...
        if ((currentSection >= 0 && currentSection != EHIndexViewInvalidSection)
            || currentSection == EHIndexViewSearchSection) {
    ...
    CGFloat insetTop = kEHIndexViewInsetTop;
...
{
    if ((self.currentSection < 0 && self.currentSection != EHIndexViewSearchSection)
        || self.currentSection >= (NSInteger)self.subTextLayers.count) {
        return;
    }
    
    CGFloat insetTop = kEHIndexViewInsetTop;
    if (self.currentSection == EHIndexViewSearchSection) {
...
- (void)showIndicator:(BOOL)animated
{
    ...
    EHTextLayer *textLayer = self.subTextLayers[self.currentSection];
    if (self.configuration.indexViewStyle == EHIndexViewStyleDefault) {
...
- (BOOL)beginTrackingWithTouch:(UITouch *)touch withEvent:(UIEvent *)event
{
    ...
    NSInteger currentPosition = EHPositionOfTextLayerInY(location.y, kEHIndexViewMargin, kEHIndexViewSpace);
...
- (void)setCurrentSection:(NSInteger)currentSection
{
    ...
    if ((currentSection < 0 && currentSection != EHIndexViewSearchSection)
