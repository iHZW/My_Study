...
- (void)viewDidLoad {
    ...
    [self loadEHIndexView];
}


- (void)loadEHIndexView {
    dispatch_async(dispatch_get_main_queue(), ^{
        
        /** 忽略 最近/热门数据  */
        NSMutableArray *tempArray = [NSMutableArray arrayWithArray:self.firstLetterMutableArrary];
        NSInteger startIndex = 0;
        if ([tempArray containsObject:self.config.popularCitiesAbbreviation]) {
            [tempArray removeObject:self.config.popularCitiesAbbreviation];
            startIndex += 1;
        }
        
        if ([tempArray containsObject:self.config.historicalRecordAbbreviation]) {
            [tempArray removeObject:self.config.historicalRecordAbbreviation];
            startIndex += 1;
        }
        
        /** 设置右侧索引数据  */
        self.tableView.eh_indexViewDataSource = tempArray.copy;
        /** 设置开始索引  */
        self.tableView.eh_startSection = startIndex;
...
- (UITableView *)tableView {
    if (!_tableView) {
        ...
        EHIndexViewConfiguration *configuration = [EHIndexViewConfiguration configurationWithIndexViewStyle:self.config.indexViewType];
        _tableView.eh_indexViewConfiguration = configuration;
        _tableView.eh_translucentForTableViewInNavigationBar = YES;
    }
    return _tableView;
}
