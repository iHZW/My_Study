...
- (void)closeControlClick{
    if (self.delegate && [self.delegate respondsToSelector:@selector(closeControlForSPModalView)]) {
        [self.delegate closeControlForSPModalView];
    }
...
- (UIControl *)closeControl {
    if (!_closeControl) {
        ...
        [closeControl addTarget:self action:@selector(closeControlClick) forControlEvents:UIControlEventTouchUpInside];
...
//    self.contentView.frame = self.coreView.bounds;
    ...
    UIBezierPath *path = [UIBezierPath bezierPathWithRoundedRect:self.contentView.bounds byRoundingCorners:UIRectCornerTopLeft | UIRectCornerTopRight cornerRadii:CGSizeMake(12, 12)];
