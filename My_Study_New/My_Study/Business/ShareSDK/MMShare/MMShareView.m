...
@interface MMShareView () <UICollectionViewDelegate, UICollectionViewDataSource>
...
- (instancetype)initWithFrame:(CGRect)frame {
    ...
    if (self) {
        ...
        _containViewColor               = UIColor.cyanColor;
        ...
        maskView.backgroundColor = UIColorFromRGBA(0x000000, 0.5);
        maskView.tag             = 100;
        ...
        _cancleButton.frame           = CGRectMake(0, 0, frame.size.width, 50 + 34);
...
- (instancetype)initWithItems:(NSArray *)items countEveryRow:(NSInteger)count {
    ...
    if (self) {
        self.shareItems       = items;
        self.itemSize         = CGSizeMake(kMainScreenWidth / count, kMainScreenWidth / count);
        self.oneLine          = NO;
...
- (void)layoutSubviews {
    ...
    // 动画前置控件位置
    if (_containView) {
        _containView.frame = CGRectMake(0, kMainScreenHeight, kMainScreenWidth, height);
    }
    if (_headerView) {
        _headerView.frame = CGRectMake(0, 0, kMainScreenWidth, _headerView.frame.size.height);
    }
    if (_bodyView) {
        float bodyY     = _headerView ? CGRectGetMaxY(_headerView.frame) : 0;
        _bodyView.frame = CGRectMake(_bodyViewEdgeInsets.left, bodyY + _bodyViewEdgeInsets.top, kMainScreenWidth - _bodyViewEdgeInsets.left - _bodyViewEdgeInsets.right, bodyHeight);
    ...
    if (_footerView) {
        _footerView.frame = CGRectMake(0, CGRectGetMaxY(_bodyView.frame) + _bodyViewEdgeInsets.bottom, kMainScreenWidth, _footerView.frame.size.height);
    }

    if (_cancleButton) {
        _cancleButton.frame = CGRectMake(0, height - _cancleButton.frame.size.height, kMainScreenWidth, _cancleButton.frame.size.height);
    }
    ...
    [UIView animateWithDuration:0.25 animations:^{
        if (self->_containView) {
            self->_containView.frame = CGRectMake(0, kMainScreenHeight - height, kMainScreenWidth, height);
        }
...
- (void)tappedCancel {
    [UIView animateWithDuration:0.25 animations:^{
        ...
        if (self.containView) {
            self.containView.frame = CGRectMake(0, kMainScreenHeight, kMainScreenWidth, self.containView.frame.size.height);
        }
...
- (void)showInView:(UIView *)view {
    ...
    NSInteger numberOfPerRow = kMainScreenWidth / _itemSize.width;
    ...
    UICollectionView *shareCollectionView              = [[UICollectionView alloc] initWithFrame:CGRectMake(0, 0, kMainScreenWidth, height) collectionViewLayout:flowLayout];
    ...
    [shareCollectionView registerClass:[MMShareItemCell class] forCellWithReuseIdentifier:kCellIdentifier_MMShareItemCell];
    ...
    if (self.functionItems) {
        ...
        [functionCollectionView registerClass:[MMShareItemCell class] forCellWithReuseIdentifier:kCellIdentifier_MMShareItemCell];
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    MMShareItemCell *shareItemCell = [collectionView dequeueReusableCellWithReuseIdentifier:kCellIdentifier_MMShareItemCell forIndexPath:indexPath];
    ...
    if (collectionView == self.shareCollectionView) {
        if ([self.shareItems[indexPath.row] isKindOfClass:[NSString class]]) {
            shareItemCell.shareItem = [[MMShareItem alloc] initWithPlatformName:self.shareItems[indexPath.row]];
        } else {
        ...
        if ([self.functionItems[indexPath.row] isKindOfClass:[NSString class]]) {
            shareItemCell.shareItem = [[MMShareItem alloc] initWithPlatformName:self.functionItems[indexPath.row]];
        } else {
...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath {
    MMShareItemCell *cell = (MMShareItemCell *)[collectionView cellForItemAtIndexPath:indexPath];
    if (cell.shareItem.action) {
        cell.shareItem.action(cell.shareItem);
    }
    
    if (self.action) {
        self.action(cell.shareItem);
    }
    [self dismiss:YES];
}
