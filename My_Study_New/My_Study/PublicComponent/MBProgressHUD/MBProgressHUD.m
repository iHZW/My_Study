...
- (void)updateIndicators {
    ...
    if(mode == MBProgressHUDModeForPAS){
        [self.indicator removeFromSuperview];
        self.indicator = [[PASMBProgressView alloc] init];
        
        NSLog(@"kMainScreenWidth = %f", kMainScreenWidth);
...
#pragma mark - PASMBProgressView
@implementation PASMBProgressView
...
- (id)initWithFrame:(CGRect)frame {
    ...
    if (self) {
        ...
        self.animateTime = 1.2;
        _bgImgView = [[UIImageView alloc] initWithFrame:self.frame];
        [_bgImgView setImage:[UIImage imageNamed:@"loadingBgImage"]];
        [self addSubview:_bgImgView];
        
        CGFloat width = self.frame.size.width;
        CGFloat height = self.frame.size.height;
        float imgWidth = MIN(width, height)*1/2;
        _paImgView = [[UIImageView alloc] initWithFrame:CGRectMake((width-imgWidth)/2, (height-imgWidth)/2, imgWidth, imgWidth)];
        [_paImgView setImage:[UIImage imageNamed:@"loadingPAImage"]];
        [self addSubview:_paImgView];
...
- (void)showAnimate{
    ...
    rotationAnimation.duration = self.animateTime;
    rotationAnimation.cumulative = YES;
    rotationAnimation.repeatCount = HUGE_VALF;
    [_bgImgView.layer addAnimation:rotationAnimation forKey:@"rotationAnimation"];
}
